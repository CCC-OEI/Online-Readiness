/*
 * Keyboard handler for tab panel
 *
 * Modified from Open Ajax Alliance Tab Panel example:
...
tabpanel.prototype.switchTabs = function($curTab, $newTab) {
  ...
  if (this.accordion == false) {
    ...
    if ($newTab.attr('id') === 'tab-video') {
      $('#content').show().attr('aria-hidden', 'false');
      $('#tabs-wrapper').css('height','2em');
    }
    else {
      if (mobile) {
        // hide video when video tab does not have focus
  ...
  // add tab hash to URL so position can be preserved on browser refresh
  var hash = $newTab.attr('id').substr(4);
...
tabpanel.prototype.handleTabClick = function($tab, e) {
  ...
  if (mobile) {
    if (tab === 'video') {
      ...
      $('#content').hide().attr('aria-hidden', 'true');;
      $('#tabs-wrapper').css('height','auto');
      $('#panel-' + tab).attr('aria-hidden','false').show();
    }
  }
  else {
    $('#panel-' + tab).attr('aria-hidden','false').show();
